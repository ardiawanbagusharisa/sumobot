...
class AIBot_ML_LSTM : Bot
{
    public override string ID => "LSTM";
    ...
    public override void OnBotUpdate()
    {
        if (buffer.Count >= 5)
        {
            List<float[]> result = buffer.Take(5).ToList();
            float[] inputs = result.SelectMany((x) => x).ToArray();
            Debug.Log($"Input total: {inputs.Length}");
            RunLSTM(inputs);
            buffer.Clear();
        }
    ...
    private void CreateEngine()
    {
        ...
        {
            ModelAsset modelAsset = Resources.Load("Models/ML/lstm") as ModelAsset;
