...
namespace SumoBot
{
    ...
    {
        public float ScriptInterval = 0.2f;
    ...
    public class AIBot_EA_GA : Bot
    {
        ...
        {
            brain = Run();
            Enqueue(brain.GetBestActionIndex());
            base.OnBotUpdate();
        }
        ...
        public float Evaluate(AIBot_GA_Genome genome)
        {
            ...
            ISumoAction action = genome.GetBestActionIndex();

            SimulateResultAPI simResult = api.Simulate(action);
            Vector3 aiDir = simResult.Rotation;
            Vector3 aiPos = simResult.Position;

            if (Vector3.Distance(aiPos, api.BattleInfo.ArenaPosition) > api.BattleInfo.ArenaRadius)
                return -999f;
